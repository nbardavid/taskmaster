...
pub fn parse(config: *Config, file_reader: *Io.Reader) ![]const Job {
    const content = try file_reader.allocRemaining(config.allocator(), .unlimited);
    const parsed = try json.parseFromSliceLeaky([]const Job, config.allocator(), content, .{});
    try config.jobs.ensureTotalCapacityPrecise(config.allocator(), parsed.len);
    config.jobs.appendSliceAssumeCapacity(parsed);
    return config.jobs.items;
}
